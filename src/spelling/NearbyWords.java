...
public class NearbyWords implements SpellingSuggest {
	 ...
	 */
		for (int index = 0; index < s.length(); index++) {
			...
			// Two criteria: (a) if the item isn't on the list, and (b) (if wordsOnly is true)
			// item is a real word, add to the list
			if(!currentList.contains(sb.toString()) &&
				(!wordsOnly || dict.isWord(sb.toString()))) {
				currentList.add(sb.toString());
			}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		//remove word from the queue, calculate distanceOne() mutation words and add back to the queue
		int threshold = 0;
		while (!queue.isEmpty() && retList.size() < numSuggestions && threshold < THRESHOLD) {
			...
			List<String> currNeighbor = distanceOne(curr, false);
			for (String n : currNeighbor) {
