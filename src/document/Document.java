...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		// System.out.println("Counting syllables in " + word + " : "):;
		int numSyllables = 0;
		...
		for (int i= 0; i < cArray.length; i++) {
			char c = cArray[i];
		    if (i == cArray.length-1 && Character.toLowerCase(c) == 'e' 
		    		&& newSyllable && numSyllables > 0) {
						numSyllables--;
			}		
			if (newSyllable && vowels.indexOf(Character.toLowerCase(c)) >= 0) {
				newSyllable = false;
				numSyllables++;
			}
			else if (vowels.indexOf(Character.toLowerCase(c)) < 0) {
				newSyllable = true;
			}
		}
